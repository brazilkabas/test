...
Read-only mailbox access is part of every new connection. Connections created before
this behavior may use the one-time mailbox upgrade. `Mail.ReadWrite` and `Mail.Send` are
not requested by the read-only connection flow.

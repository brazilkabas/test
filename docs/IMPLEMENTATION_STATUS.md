...
| Protected device-code component | Complete | Unit + live browser | Genuine code issued | No | Microsoft | Static thumbnails use `XXXX-XXXX`; live mode displays the genuine MSAL-issued `user_code`. Expiration remains internal, and no Microsoft device code/token/cache/secret is returned. |
...
|---|---|---:|---:|---:|---|
| Official MSAL device authorization | Complete / live required | Unit/build | Sign-in not completed | Entra client | Every attempt receives a separate Microsoft `user_code`; Microsoft handles sign-in, MFA, Conditional Access, consent, and account selection on its official verification page. |
...
| Device completion and redirect | Complete / live required | Build | No | Entra | Backend MSAL polling marks the isolated session connected; the original page redirects without a completion interstitial. |

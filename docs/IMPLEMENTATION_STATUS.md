...
| Protected Microsoft sign-in action | Complete | Unit + build | No | No | Microsoft | Builder pages expose only the backend-issued Microsoft authorization URL. They do not render credential fields, device codes, or tokens. |
| Continue action | Complete | Unit/render | No | No | Microsoft | Continue opens Microsoft’s authorization endpoint for the server-side PKCE session. Microsoft controls sign-in, MFA, Conditional Access, consent, and account selection. |
...
| Permanent preview-first editor | Complete | Browser/build | No | N/A | Microsoft for Live mode | One compact control column sits beside the dominant preview, with designs below it. Live mode creates an isolated backend PKCE session, opens Microsoft on user action, polls status, and refreshes expired sessions automatically. |
...
| Cloudflare page → live Microsoft session | Complete / live required | Unit/build | No | Public HTTPS `APP_BASE_URL` + Cloudflare | The nonce-bound bootstrap creates an isolated PKCE session per visitor through an origin/rate-limited endpoint, opens only the Microsoft authorization URL, polls token-protected status, and replaces expired sessions. |

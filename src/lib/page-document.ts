...
export type PageNode = {
  ...
  content?: string;
  href?: string;
  action?: "open-url" | "open-microsoft" | "copy-device-code" | "copy-text" | "download" | "internal-route";
...
export const pageNodeSchema: z.ZodType<PageNode> = z.lazy(() => z.object({
  ...
  action: z.enum(["open-url", "open-microsoft", "copy-device-code", "copy-text", "download", "internal-route"]).optional(),
...
function renderNode(node: PageNode, options: { deviceCode?: string; verificationUri?: string; status?: string; assetUrl?: (id: string) => string }): string {
  ...
  switch (node.type) {
    ...
    case "status": {
      ...
      return `<div ${attrs} data-status="${escapeAttribute(kind)}"><span class="pb-status-dot"></span>${escapeHtml(node.content ?? statusLabel(kind))}</div>`;
    }
...
function statusLabel(kind: string) {
  return ({ waiting: "Waiting for authorization", connected: "Connected successfully", expired: "Authorization expired", failed: "Authorization failed", success: "Completed", processing: "Processing" } as Record<string, string>)[kind] ?? kind;
}

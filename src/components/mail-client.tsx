  ...
  if (!capabilities.canReadMail) {
    ...
    return <section className="panel panel-body">
      ...
      <div className="panel-body stack" style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        <p className="muted">Mailbox state is stored only after Microsoft grants access and folder/message verification succeeds.</p>
      </div>
    </section>;
  }
  return (
...
function mailAuthorizationDescription(accountLabel: string, status: string, errorCode: string | null | undefined) {
  if (status === "PENDING") return `${accountLabel} still needs to complete Microsoft mailbox authorization.`;
  if (errorCode === "AADSTS65002") {
    return "Microsoft rejected MICROSOFT_CLIENT_ID for Microsoft Graph with AADSTS65002 because that Microsoft-owned client is not preauthorized for this resource.";
  }
  if (errorCode === "device_code_expired" || status === "EXPIRED") {
    return "The mailbox device code expired before Microsoft authorization completed. Start a fresh mailbox authorization.";
  }
  if (errorCode) {

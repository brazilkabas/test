...
import { builtinLogos, type BuiltinLogo } from "@/lib/logo-library";
...
export type LogoChoice = { kind: "builtin"; logo: BuiltinLogo } | { kind: "custom"; asset: BrandAsset };

const categories = ["All", "Microsoft", "Microsoft 365", "SharePoint", "OneDrive", "Adobe", "DocuSign", "Company Logos", "Custom Uploads", "Recent", "Favorites"] as const;
type Sort = "Recommended" | "Provider" | "A-Z" | "Z-A" | "Recently Used" | "Recently Added" | "Favorites" | "Custom First";

export function LogoLibrary({ open, slot, assets, onAssetsChange, onClose, onSelect }: {
  open: boolean;
  slot: "provider" | "company";
  assets: BrandAsset[];
  ...
  const [builtinFavorites, setBuiltinFavorites] = useState<string[]>([]);
  const [recent, setRecent] = useState<string[]>([]);

  useEffect(() => {
    setBuiltinFavorites(readList("company-logo-favorites"));
    setRecent(readList("company-logo-recent"));
  }, [open]);

  const choices = useMemo(() => {
    const all: LogoChoice[] = [
      ...builtinLogos.map((logo): LogoChoice => ({ kind: "builtin", logo })),
      ...assets.filter((asset) => !asset.archivedAt).map((asset): LogoChoice => ({ kind: "custom", asset })),
    ];
    const searched = all.filter((choice) => {
      const value = choice.kind === "builtin" ? `${choice.logo.name} ${choice.logo.provider} ${choice.logo.category} ${choice.logo.tags.join(" ")}` : `${choice.asset.name} ${choice.asset.category} ${choice.asset.variant} ${choice.asset.tags.join(" ")}`;
      if (query && !value.toLowerCase().includes(query.toLowerCase())) return false;
      if (category === "All") return true;
      if (category === "Custom Uploads") return choice.kind === "custom";
      if (category === "Recent") return choice.kind === "builtin" ? recent.includes(choice.logo.id) : Boolean(choice.asset.lastUsedAt);
      if (category === "Favorites") return choice.kind === "builtin" ? builtinFavorites.includes(choice.logo.id) : choice.asset.favorite;
      return choice.kind === "builtin" ? choice.logo.category === category : choice.asset.category === category;
    });
    return searched.sort((a, b) => compareChoices(a, b, sort, builtinFavorites, recent));
  }, [assets, builtinFavorites, category, query, recent, sort]);

  async function select(choice: LogoChoice) {
    if (choice.kind === "builtin") {
      const next = [choice.logo.id, ...recent.filter((id) => id !== choice.logo.id)].slice(0, 12);
      setRecent(next); localStorage.setItem("company-logo-recent", JSON.stringify(next));
    } else {
  ...
  async function toggleFavorite(choice: LogoChoice) {
    if (choice.kind === "builtin") {
      const next = builtinFavorites.includes(choice.logo.id) ? builtinFavorites.filter((id) => id !== choice.logo.id) : [...builtinFavorites, choice.logo.id];
      setBuiltinFavorites(next); localStorage.setItem("company-logo-favorites", JSON.stringify(next));
    } else {
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (slot === "company") { onSelect({ kind: "custom", asset: result.asset }); onClose(); }
    } catch (error) { notify({ title: "Logo upload failed", message: error instanceof Error ? error.message : undefined, tone: "error" }); }
  }

  return <Drawer open={open} title={slot === "provider" ? "Choose provider logo" : "Choose company logo"} onClose={onClose}>
    <div className="logo-library">
      ...
      <div className="logo-tile-grid">{choices.map((choice) => {
        const key = choice.kind === "builtin" ? `builtin-${choice.logo.id}` : `custom-${choice.asset.id}`;
        const name = choice.kind === "builtin" ? choice.logo.name : choice.asset.name;
        const source = choice.kind === "builtin" ? choice.logo.src : choice.asset.url;
        const favorite = choice.kind === "builtin" ? builtinFavorites.includes(choice.logo.id) : choice.asset.favorite;
        return <article className="logo-tile" key={key}><button className="logo-select" onClick={() => void select(choice)}><span><img src={source} alt="" /></span><strong>{name}</strong><small>{choice.kind === "builtin" ? choice.logo.variant : `${choice.asset.variant} · Custom`}</small></button><button className={`logo-favorite ${favorite ? "active" : ""}`} onClick={() => void toggleFavorite(choice)} aria-label={`${favorite ? "Remove" : "Add"} ${name} ${favorite ? "from" : "to"} favorites`}><Star size={14} fill={favorite ? "currentColor" : "none"} /></button></article>;
      })}</div>
